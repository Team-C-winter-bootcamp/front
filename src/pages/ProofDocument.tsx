...
export default function ProofDocument() {
  ...
  useEffect(() => {
    const initDocument = async () => {
      ...
      try {
        const response = await caseService.generateDocument('notice', {
  ...
  const handleSend = async () => {
    ...
    setIsStreaming(true);
    try {
      const response = await caseService.updateDocument('notice', {
        ...
        <div className="flex-1 bg-slate-100 p-8 overflow-auto pb-40">
          {isGenerating ? (
            <div className="h-full flex flex-col items-center justify-center">
              ...
              <p className="font-bold text-slate-600 text-lg">AI가 내용증명을 작성 중입니다...</p>
            </div>
          ) : (
        ...
        <div ref={chatContainerRef} className="bg-white border-t border-gray-200 flex flex-col absolute bottom-0 w-full z-10 shadow-lg" style={{ height: `${chatHeight}px` }}>
          ...
          <div className="p-4 border-t bg-white">
            <div className="max-w-4xl mx-auto flex gap-2">
              <input className="flex-1 border border-gray-200 rounded-xl px-4 py-3 outline-none focus:border-indigo-500 text-sm shadow-sm" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="내용증명에 추가할 내용을 입력하세요..." />
